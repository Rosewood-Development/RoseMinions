...
import static dev.rosewood.roseminions.setting.DataSerializers.setWithComments;

@SuppressWarnings("unchecked")
final class DataSerializerFactories {

    private DataSerializerFactories() { }

    //region Serializer Factories
    public static <T extends Enum<T>> DataSerializer<T> ofEnum(Class<T> enumClass) {
        return new DataSerializer<>(enumClass, MinionPersistentDataType.forEnum(enumClass), x -> x.name().toLowerCase(), x -> Enum.valueOf(enumClass, x.toUpperCase())) {
    ...
    public static <T extends Keyed> DataSerializer<T> ofKeyed(Class<T> keyedClass, Function<NamespacedKey, T> valueOfFunction) {
        return new DataSerializer<>(keyedClass, MinionPersistentDataType.forKeyed(keyedClass, valueOfFunction), x -> translateName(x.getKey()), x -> valueOfFunction.apply(translateKey(x))) {
    ...
    public static <T> DataSerializer<T[]> ofArray(DataSerializer<T> serializer) {
        return new DataSerializer<>(MinionPersistentDataType.forArray(serializer.persistentDataType)) {
    ...
    public static <T> DataSerializer<List<T>> ofList(DataSerializer<T> serializer) {
        return new DataSerializer<>(MinionPersistentDataType.forList(serializer.persistentDataType)) {
    ...
    public static <K, V> DataSerializer<Map<K, V>> ofMap(DataSerializer<K> keySerializer, DataSerializer<V> valueSerializer) {
        return new DataSerializer<>(MinionPersistentDataType.forMap(keySerializer.persistentDataType, valueSerializer.persistentDataType)) {
    ...
    public static <T, M> DataSerializer<T> ofFieldMapped(Class<T> type, String fieldKey, DataSerializer<M> fieldSerializer, Map<M, DataSerializer<? extends T>> mapper) {
        return new DataSerializer<>(type, null) {
            @Override
            public void write(ConfigurationSection config, String key, T value, String... comments) {
                DataSerializer<T> serializer = this.mapField(fieldSerializer.read(config, fieldKey));
                if (serializer != null)
            ...
            public void write(PersistentDataContainer container, String key, T value) {
                DataSerializer<T> serializer = this.mapField(fieldSerializer.read(container, fieldKey));
                if (serializer != null)
            ...
            private DataSerializer<T> mapField(M value) {
                if (value == null)
                    return null;
                try {
                    DataSerializer<? extends T> serializer = mapper.get(value);
