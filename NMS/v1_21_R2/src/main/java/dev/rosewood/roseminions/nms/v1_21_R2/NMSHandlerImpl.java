...
public class NMSHandlerImpl implements NMSHandler {
    ...
    public List<ItemStack> getFishingLoot(Entity looter, Location location, ItemStack fishingRod) {
        ...
        Registry<LootTable> registry = MinecraftServer.getServer().registryAccess().lookupOrThrow(Registries.LOOT_TABLE);
        ResourceKey<LootTable> lootTableKey = ResourceKey.create(Registries.LOOT_TABLE, resourceLocation);
        return registry.getOrThrow(lootTableKey).value().getRandomItems(params).stream()
                .filter(x -> !x.isEmpty())
